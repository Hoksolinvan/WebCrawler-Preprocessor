      ...
      {  int i = 0;
        while(TRUE)
         {  if (i == i_max) increase_s();
            ...
            if (!LETTER(ch)) { 
		//Van here I just excluded the print statement and assumed all the characters are letters
 		break; 
	}
         }
...
// I got rid of the for loop as it is should not be accepting any arguments

   s = (char *) malloc(i_max+1);
// since stdin is also a pointer of type FILE *f I replaced the original file with the stdin stream instead!
	FILE *f = stdin;
      if (f == NULL) { printf("There was nothing from STDIN"); exit(1); }
      stemfile(f);
   free(s);
   return 0;
}
